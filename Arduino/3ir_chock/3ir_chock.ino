...
#define L_S A3 //ir sensor Right
#define M_S A2 //ir sensor Left
#define R_S A1   //ir sensor Middle
...
void loop()
{  
if((digitalRead(L_S) == 0)&&(digitalRead(R_S) == 0)&& (digitalRead(M_S)== 1)){forword();}   
...
if((digitalRead(L_S) == 1)&&(digitalRead(R_S) == 1)&&(digitalRead(M_S) == 1)){turnLeft();} 
// If all sensor are on black color ( juntion point) then it will turn left
  
if((digitalRead(L_S) == 0)&&(digitalRead(R_S) == 0 )&&( digitalRead(M_S) == 0)){Uturn();} 
...
digitalWrite(in2, LOW);  //Left Motor backword Pin 
digitalWrite(in3, LOW); //Right Motor backword Pin 
digitalWrite(in4, HIGH);  //Right MotoXr forword Pin 
}
