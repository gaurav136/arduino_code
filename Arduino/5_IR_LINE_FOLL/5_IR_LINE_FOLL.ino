...
#define ExR_S A1 //ir sensor Right
#define R_S A2 //ir sensor Left
#define M_S A3
#define L_S A4
#define ExL_S A5 //ir sensor Middle

void setup(){ // put your setup code here, to run once

pinMode(ExR_S, INPUT); // declare if sensor as input 
pinMode(R_S, INPUT); // declare if sensor as input
pinMode(M_S, INPUT); // declare if sensor as input
pinMode(L_S, INPUT); // declare ir sensor as input
pinMode(ExR_S, INPUT); // declare ir sensor as input 
...
void loop()
{  
if(((digitalRead(ExL_S) == 0)||(digitalRead(L_S) == 0))&&( digitalRead(M_S) == 1)&&((digitalRead(R_S)==0)||(digitalRead(ExR_S)==0))){forword();}   
...
if(((digitalRead(ExL_S)==1)||(digitalRead(L_S) == 1))&&((digitalRead(R_S) == 0)||(digitalRead(ExR_S)==0))){turnLeft();}
//if left Sensor is Black and right Sensor is White then it will call turn Right function  

if(((digitalRead(ExL_S)==0)||(digitalRead(L_S) == 0))&&((digitalRead(R_S) == 1)||(digitalRead(ExR_S)==1))){turnRight();}  
//if left Sensor is White and right Sensor is Black then it will call turn Left function

if(((digitalRead(ExL_S)==1)||(digitalRead(L_S) == 1))&&((digitalRead(R_S) == 1)||digitalRead(ExR_S))&&(digitalRead(M_S) == 1)){turnLeft();} 
// If all sensor are on black color ( juntion point) then it will turn left
  
if((digitalRead(ExL_S) == 0)&&(digitalRead(L_S)==0)&&(digitalRead(R_S) == 0 )&&(digitalRead(ExR_S)==0)&&( digitalRead(M_S) == 0)){Uturn();} 
